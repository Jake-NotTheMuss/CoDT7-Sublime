...
def replace_rows_with_template(view, edit, template):
    ...
    # position to put cursor after inserting template
    final_cursor_pos = view.sel()[0].begin()
    # list of new selections to add afterward
    new_selections = []
    for region in view.sel():
        debug('Inserting template into selection', region)
        ...
        debug('Inserting template at position', line_pos, 'in line')
